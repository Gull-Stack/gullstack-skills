...
**Status (2026-07-18):** Repo-side rebrand and the full draft legal package are complete on branch `claude/goldstack-501c3-migration-4v0l9a`, incorporating the owner's confirmed decisions: **(1)** counsel will review and file using these drafts; **(2)** the asset transfer is structured as a **charitable donation** (qualified appraisal + Form 8283 required); **(3)** scope is **mission assets only** — Plato's Closet and market-rate client contracts are excluded from the c3, per the exclusions in the assignment agreement. Next actions are with counsel/CPA: review drafts, fill registered agent / directors / assignor-entity blanks, sign and file articles → EIN → bank account → Form 1023. Then run `CUTOVER-RUNBOOK.md`.
